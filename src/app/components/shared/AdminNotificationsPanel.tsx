...
function getNotificationIcon(type: Notification['type']) {
  switch (type) {
    case 'new_codes_to_price':
      return <ListChecks size={18} style={{ color: '#78BE20' }} />;
    case 'support_reply':
...
export function AdminNotificationsPanel({ plaza }: AdminNotificationsPanelProps) {
  const {
  ...
  const notifications = getNotificationsForRole('admin', plaza);
  const unreadCount = getUnreadCount('admin', plaza);

  const codeNotifications = notifications.filter(
    (n) => n.type === 'new_codes_to_price'
  );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        {unreadCount > 0 && (
          <button
            onClick={() => markAllAsRead('admin', plaza)}
            style={{
              display: 'flex',
              alignItems: 'center',
              gap: '6px',
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
          <ListChecks size={16} style={{ color: '#78BE20' }} />
          <h4 style={{ fontSize: '14px', fontWeight: 600, color: '#001022', margin: 0 }}>
            Novos Códigos para Precificar
          </h4>
        ...
        {codeNotifications.length === 0 ? (
          ...
          >
            <ListChecks size={24} style={{ color: '#D1D5DB', marginBottom: '8px' }} />
            <p style={{ fontSize: '13px', color: '#9CA3AF' }}>Nenhum novo código para precificar</p>
          </div>
        ) : (
